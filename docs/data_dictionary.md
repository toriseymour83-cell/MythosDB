...
Stores ancient authors, attributed authors, Roman authors, and later source writers used in the dataset.
...
Stores texts, plays, poems, mythographic works, histories, and source collections.
...
Stores gods, heroes, mortals, monsters, rulers, creatures, and other mythological figures.
...
| `character_type` | Broad category, such as Olympian God, Hero, Mortal, or Monster |
...
| `location_type` | City, island, realm, mountain, region, sanctuary, etc. |
...
| `relationship_type` | Type of link, such as `parent_of`, `killed`, or `ally_of` |
...
| `category` | Broad group, such as Family, Conflict, Alliance, or Journey |
...
Stores alternative names, Roman names, titles, epithets, and spelling variants.
...
| `alias_type` | Roman name, Greek form, title, epithet, spelling variant, etc. |
...
| `entity_type` | Table or entity type, such as character, event, location, or relationship |
...
## `data/crimes_and_punishments.csv`

Stores selected mythological actions, punishments, and simplified modern UK legal comparisons.

This table is intended for data analysis and discussion only. It is not legal advice.

| Column | Description |
|---|---|
| `crime_id` | Unique ID |
| `event_id` | Links to `events.event_id`, where applicable |
| `character_id` | Links to `characters.character_id`, where applicable |
| `alleged_action` | Description of the action |
| `mythological_category` | Category of mythological action |
| `mythological_punishment` | Punishment or consequence in the myth |
| `punishing_figure` | Character, deity, social force, or consequence linked to punishment |
| `modern_uk_equivalent` | Simplified modern UK legal comparison |
| `likely_illegal_today` | `TRUE`, `FALSE`, or `UNCLEAR` |
| `modern_law_note` | Explanation of legal uncertainty or comparison |
| `severity_rating` | Indicative rating from 1 to 5 |
...
Greek mythology contains variant traditions, disputed authorship, uncertain dates, and regional differences.

The dataset uses notes, source references, and confidence fields to make uncertainty visible rather than presenting every record as fixed or definitive.

---

## Notes on IDs

Most tables use integer ID fields so that the data can be joined in SQL, Power BI, Excel, or Python.
